...
//FUNCOES PARA CASA--------------------------------------------------------------------------------------------//
house *aloca_casa(int idbairro, int idrua, int idconsumidor, int casa, char *consumidor)
{
...
house *insereinicio_casa(nhood *head_bairro, int idbairro, int idrua, int idconsumidor, int casa, char *consumidor)
{
    street *rua = procura_rua(head_bairro, idrua, idbairro);
    if(rua != NULL)
    {
        house *nova = aloca_casa(idbairro, idrua, idconsumidor, casa, consumidor);
        house *old_primeira;
...
house *insere_casa(nhood *head_bairro, int idbairro, int idrua, int idconsumidor, int casa, char *consumidor)
{
    ...
    if(rua != NULL)
    {
        ...
        else
        {
           ...
           {
              return insereinicio_casa(head_bairro, idbairro, idrua, idconsumidor, casa, consumidor);
           } 
           else 
           {   
...
float medir_rua(nhood *head_bairro, int idbairro, int idrua)
{
    float consumo_rua=0;
    street *rua = procura_rua(head_bairro, idrua, idbairro);
    printf("AChou a rua, mamãe! : %i\n", rua->id_rua);
    printf("Casa %i na rua 3\n", rua->head_casa->id_casa);
    if(rua != NULL)
    {  
      printf("COnseguiu entrar no if rua\n");
      house *aux;
      aux = rua->head_casa;
      printf("ID da casa antes de entrar: %i\n", rua->head_casa->id_casa);
      printf("ID da casa antes de entrar aux: %i\n", aux->id_casa);
      while(aux->proxc != NULL)
      {
          consumo_rua += aux->consumo;
          printf("ID da casa analisada %i\n", aux->id_casa);
          aux = aux->proxc;
      }
      return consumo_rua;
    }
...
float medir_bairro(nhood *head_bairro, int idbairro)
{
    ...
    if(bairro!= NULL)
    {
       ...
       {
        printf("Nao há ruas nesse bairro!\n");
        return 0;
       }
    }
    else return 0;
...
float medir_cidade(nhood * head_bairro)
{
  float total = 0;
  nhood *proximo = head_bairro->proxb->proxb;

  printf("%i %i\n", proximo->id_bairro, proximo->proxb->id_bairro);

  while (proximo != NULL)
  {
    printf("Entrou!\n");
    total = total + medir_bairro(head_bairro, proximo->id_bairro);
    printf("Medir cidade: %i\n", proximo->id_bairro);
    proximo = proximo->proxb;
  }
